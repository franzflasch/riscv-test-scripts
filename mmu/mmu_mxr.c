...
#if (__riscv_xlen == 64)
    #include "../generated_files/rv64/compiled_files/mmu_supervisor_mxr_rv64.c"

    #define SUPERVISOR_PROGRAM generated_files_rv64_compiled_files_mmu_supervisor_mxr_rv64_bin
    #define SUPERVISOR_PROGRAM_LEN generated_files_rv64_compiled_files_mmu_supervisor_mxr_rv64_bin_len
#else
    #include "../generated_files/rv32/compiled_files/mmu_supervisor_mxr_rv32.c"

    #define SUPERVISOR_PROGRAM generated_files_rv32_compiled_files_mmu_supervisor_mxr_rv32_bin
    #define SUPERVISOR_PROGRAM_LEN generated_files_rv32_compiled_files_mmu_supervisor_mxr_rv32_bin_len
#endif
...
uint32_t Sv32_mmu_map_virt_exec_only(uint32_t virt_addr, uint32_t phys_addr, uint8_t user_page)
{
    uint32_t vpn[SV32_LEVELS] = 
    {
        (virt_addr >> 12) & 0x3ff,
    ...
    *pte = (*pte >> 2) | PAGE_VALID;
    ...
    /* Set physical address here */
    if(user_page)
        *pte = (ppn[1] << 20) | (ppn[0] << 10) | PAGE_USER | PAGE_EXEC | PAGE_VALID;
    else
        *pte = (ppn[1] << 20) | (ppn[0] << 10) | PAGE_EXEC | PAGE_VALID;
}
...
void kmain(void)
{
     ...
     */
    Sv32_mmu_map_virt_exec_only(0x2000, supervisor_phys_addr, 0);
    ...
    /* Set MXR=1 to check if page read marked as executable works */
    mstatus = (1<<19) | (1<<18) | (1<<11);
